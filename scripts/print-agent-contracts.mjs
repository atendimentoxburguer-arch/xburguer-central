...
assert.match(server,/System\.Drawing\.Printing\.PrinterSettings/);
assert.match(server,/CurrentControlSet\\\\Control\\\\Print\\\\Printers/);
assert.match(server,/getPrinterDiagnostics/);
...
assert.match(desktopMain,/openCentralAutomatically:true/);
assert.match(desktopMain,/if\(desktopConfig\.openCentralAutomatically\)createCentralWindow\(\{show:true\}\)/);
assert.match(desktopMain,/app:set-auto-open-central/);
assert.match(desktopMain,/Abrir Central automaticamente/);
...
assert.match(client,/Mapear impressora manualmente/);
assert.match(client,/Nome exato da impressora no Windows/);
assert.match(client,/Mapeada manualmente/);
assert.match(client,/function reconnectPrintAgent/);
assert.match(client,/Reconectar agente/);
assert.match(client,/event==='manual'\|\|event==='test'/);
assert.match(client,/await reconnectPrintAgent/);
