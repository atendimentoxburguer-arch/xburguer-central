...
function orderCard(o){
 ...
 return `<div class="order-card ${late?'late':''}" data-order-search="${search}" data-type="${esc(o.type)}"><div class="order-head"><span class="type-badge">${typeIcon(o.type)}</span><span class="num">Pedido #${esc(o.id)}</span><time>${icon('clock')} ${orderTime(o)}</time></div>${late?'<div class="order-late">Pedido atrasado</div>':''}<div class="order-info"><div>${esc(o.customer)}</div>${o.table?`<div class="order-address"><span class="addr-icon">${icon('table')}</span><span>${esc(o.table)}</span></div>`:''}${o.address?`<div class="order-address"><span class="addr-icon">${icon('geo-alt')}</span><span>${esc(o.address)}</span></div>`:''}<div class="order-row"><span>${esc(orderPaymentLabel(o))}</span><b>${money(orderTotal(o))}</b></div></div><div class="order-actions"><button class="btn btn-outline btn-sm" onclick="detailsOrder('${o.id}')">Gerenciar</button>${action}</div></div>`;
}
