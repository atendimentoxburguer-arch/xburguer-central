...
function renderPedidos(){
 ...
 root.innerHTML=`<div class="page-head"><div><h1>Gestão de pedidos</h1><p>Acompanhe pedidos ativos, edite lançamentos e consulte cancelamentos e vendas concluídas.</p></div><div class="page-head-actions"><button class="btn btn-primary" onclick="go('pdv')">${icon('plus-lg')}<span>Novo pedido</span></button></div></div>
 <div class="notice"><span class="notice-icon">${icon('info-circle')}</span><span>Caixa ${state.cash.open?'aberto':'fechado'}${state.cash.open?' • saldo em dinheiro '+money(cashDrawerBalance()):''}. <span class="link" onclick="go('caixa')">Abrir frente de caixa</span>.</span><button class="x" onclick="this.parentElement.remove()" aria-label="Fechar aviso">${icon('x-lg')}</button></div>
...
function orderCard(o){
 ...
 return `<div class="order-card ${late?'late':''}" data-order-search="${search}" data-type="${esc(o.type)}"><div class="order-head"><span class="type-badge">${typeIcon(o.type)}</span><span class="num">Pedido #${esc(o.id)}</span><time>${icon('clock')} ${orderTime(o)}</time></div>${late?'<div class="order-late">Pedido atrasado</div>':''}<div class="order-info"><div>${esc(o.customer)}</div>${o.table?`<div class="order-address"><span class="addr-icon">${icon('table')}</span><span>${esc(o.table)}</span></div>`:''}${o.address?`<div class="order-address"><span class="addr-icon">${icon('geo-alt')}</span><span>${esc(o.address)}</span></div>`:''}<div class="order-row"><span>${esc(o.payment)}</span><b>${money(orderTotal(o))}</b></div></div><div class="order-actions"><button class="btn btn-outline btn-sm" onclick="detailsOrder('${o.id}')">Gerenciar</button>${action}</div></div>`;
}
