...
function syncTables(){state.tables.forEach(t=>{const open=state.orders.some(o=>o.table===t.name&&!['done','cancelled'].includes(o.status));if(!open){t.status='free';t.guests=0;t.server=''}else if(t.status!=='closing')t.status='busy'})}
