...
function defaultState(){return {
 ...
 cash:{open:true,openedAt:isoAgo(240),opening:0,movements:[{id:'m1',type:'suprimento',value:100,desc:'Troco inicial',at:isoAgo(230)}]},
...
let orderFilter='all';let orderSearch='';let selectedCat='cat1';let pdvCat='all';let pdvCart=[];let pdvType='Balcão';let salaoTab='mesas';let chatId='w1';
function load(){try{state=JSON.parse(localStorage.getItem(STORAGE))||defaultState()}catch(e){state=defaultState()}normalize()}
function normalize(){state.settings=state.settings||defaultState().settings;state.orders=state.orders||[];state.products=state.products||[];state.categories=state.categories||[];state.tables=state.tables||[];state.couriers=state.couriers||[];state.cash=state.cash||defaultState().cash;state.promos=state.promos||[];state.finance=state.finance||[];state.team=state.team||[];state.chats=state.chats||[]}
function save(){try{localStorage.setItem(STORAGE,JSON.stringify(state));renderAll()}catch(e){console.error(e);toast('Não foi possível salvar os dados neste navegador.')}}
...
function setHeader(){const on=state.settings.storeOpen;document.getElementById('storeToggle').classList.toggle('on',on);document.getElementById('storeText').textContent=on?'Loja aberta':'Loja fechada';document.getElementById('footStore').textContent=on?'ABERTO':'FECHADO';document.getElementById('footStore').style.background=on?'#20be6a':'#c74444';document.getElementById('sideNewCount').textContent=state.orders.filter(o=>o.status==='analysis').length}
